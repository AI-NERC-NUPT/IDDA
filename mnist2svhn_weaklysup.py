...
import os
import numpy as np
from utils import return_mnist, return_svhn, select_labelled_data,judge_idda_func, pick_samples, weight_variable,\
    bias_variable, max_pool_3x3, conv2d, batch_norm_conv, batch_norm_fc, batch_generator
flags = tf.app.flags
####-----------------------------------------------
## learning_rate = 0.025 for num_labelled = 60
## learning_rate = 0.035 for num_labelled = 80, 100
flags.DEFINE_float('learning_rate', 0.025, "value of learning rate")
FLAGS = flags.FLAGS
N_CLASS = 10
...
data_s_im, data_s_label, data_s_im_test, data_s_label_test = return_mnist(path_mnist_train, path_mnist_test)
data_t_im, data_t_label, data_t_im_test, data_t_label_test = return_svhn(path_svhn_train, path_svhn_test)
print('load finished')
...
with graph.as_default():
    ...
    total_loss = pred_loss_source + pred_loss_target
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for t in range(num_phases):
            ...
            gen_target_batch = batch_generator(
                [data_t_im[perm, :], label_target], batch_size, shuffle=False)  #with no shuffle since it is explicitly done
            ...
            if t == 0:
                ...
                #### select labelled data from target domain with strong domain-shifts
                new_ind = pick_samples(preds_stack, threshold=1.7, lower_or_upper=False)
                print('The number of picked samples = ', len(new_ind))
                if len(new_ind) < num_labelled:
                    print('The number of picked samples is too limited')
                    LL = len(new_ind)
                    new_data = cand[new_ind[0: LL-1], :, :, :]
                    new_label = canl[new_ind[0: LL-1], :]
                else:
                    new_data = cand[new_ind[0: NewL], :, :, :]
                    new_label = canl[new_ind[0: NewL], :]

            lowerValue = 0.20 * pow(2, -t) + 0.02
            if t != 0:
                cand = data_t_im[perm, :]
                canl = data_t_label[perm, :]
