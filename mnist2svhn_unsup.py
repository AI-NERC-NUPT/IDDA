...
flags = tf.app.flags
flags.DEFINE_float('learning_rate', 0.035, "value of learnin rage")
FLAGS = flags.FLAGS
N_CLASS = 10
...
print('data loading...')
############################################################################################################
#---- The original order of returned datasets are not compatiable with the definition of function
#---- Corrected by xfuwu on Mar.26, 2018
...
# Params
num_phases = 30
num_steps = 3000 * np.ones(num_phases,dtype=int)
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for t in range(num_phases):
            ...
            if t == 0:
                ...
                rate = max(int((t + 1) / 1.0 * preds_stack.shape[0]), 60000)  # k/20 * n?: number of newly-labeled target samples
            ...
            if t != 0:
                ...
                new_data, new_label, new_ind = judge_idda_func(cand,
                                                             preds_stack[:rate, :],
                                                             lower=lowerValue, #0.08 * pow(1.2, -t) + 0.04,
                                                             num_class=N_CLASS)
                correct_new_labels = tf.equal(tf.argmax(new_label, 1), tf.argmax(canl[new_ind, :], 1))
                label_correct_ones = tf.reduce_sum(tf.cast(correct_new_labels, tf.int32))
                label_errors = new_data.shape[0] - label_correct_ones

            print('new_data_size: %d'% (new_data.shape[0]))
